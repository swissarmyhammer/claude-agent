...
fn classify_error(error: &anyhow::Error) -> i32 {
    ...
    if error_msg.contains("configuration") || error_msg.contains("config") {
        EX_CONFIG
    } else if error_msg.contains("permission") || error_msg.contains("access") || error_msg.contains("denied") {
        EX_NOPERM
...
mod tests {
    ...
    fn test_cli_parsing() {
        ...
        // Test info command
        let matches = cmd.clone().try_get_matches_from(vec!["claude-agent", "info"]);
        assert!(matches.is_ok());
    }
    ...
    fn test_serve_command_parsing() {
        let cli = Cli::try_parse_from(vec!["claude-agent", "serve", "--port", "8080", "--dev"]).unwrap();
    ...
    fn test_config_command_parsing() {
        let cli = Cli::try_parse_from(vec!["claude-agent", "config", "/path/to/config.json"]).unwrap();
    ...
    async fn test_config_loading_invalid_format() {
        ...
        tokio::fs::write(&config_path, "invalid config").await.unwrap();
